                    ...
                    return (
                        <div key={item.id} className={`glass-panel p-6 rounded border flex flex-col justify-between transition-all duration-300 ${styles.border} ${styles.glow} ${hasItem ? 'opacity-50 grayscale' : 'hover:scale-[1.02]'} ${isLevelLocked ? 'opacity-70 bg-gray-900/40 border-gray-700/30' : ''}`}>
                            ...
                            <div className="mt-4 flex justify-between items-center pt-4 border-t border-white/5">
                                ...
                                <button 
                                    disabled={hasItem || !canAfford || isLevelLocked} 
            ...
            case 'menu': return (
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 animate-fadeIn pb-24 lg:pb-0">
                    {fullNav.filter(n => n.id !== 'menu').map(n => (
                        <button key={n.id} onClick={() => setPage(n.id as Page)} className="bg-[#020617] p-4 md:p-8 rounded-lg flex flex-col items-center justify-center hover:bg-blue-500/10 transition-all border-blue-500/20 group hover:border-blue-400/50 shadow-[0_8px_32px_0_rgba(0,0,0,0.4)]">
