...
// --- MAIN COMPONENT ---
export const QuestHistory: React.FC<QuestHistoryProps> = ({ completedQuests, dungeonHistory }) => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            cells.push(
                <div key={day} className="flex items-center justify-center h-14">
                    <button 
                        onClick={() => { if(hasActivity) setSelectedDate(date); }}
    ...
    return (
        <div>
            ...
            {selectedDate && activitiesByDate.has(`${selectedDate.getFullYear()}-${(selectedDate.getMonth() + 1).toString().padStart(2, '0')}-${selectedDate.getDate().toString().padStart(2, '0')}`) && (
                <HistoryDetailModal 
                    date={selectedDate} 
                    activities={activitiesByDate.get(`${selectedDate.getFullYear()}-${(selectedDate.getMonth() + 1).toString().padStart(2, '0')}-${selectedDate.getDate().toString().padStart(2, '0')}`)!} 
